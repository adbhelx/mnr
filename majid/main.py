...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query:
        await update.callback_query.edit_message_text("مرحبًا! اختر قسمًا:", reply_markup=InlineKeyboardMarkup(build_main_menu()))
    else:
...
async def adm_add_cont(update: Update, context):
    ...
    try:
        ...
        await update.message.reply_text("⚠️ حدث خطأ أثناء إضافة المحتوى.")
...
async def adm_receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"Attempting to save data for section {sec}, item {name} with file_id {fid}")
        save()
        logger.info("Data saved successfully.")
        await msg.reply_text(f"✅ تم حفظ الملف في {sec}:\n`{name}`\nfile_id=`{fid}`", parse_mode="Markdown")
    except Exception as e:
        logger.error(f"Error receiving file: {e}")
        await msg.reply_text("⚠️ حدث خطأ أثناء حفظ الملف.")
    return ConversationHandler.END
